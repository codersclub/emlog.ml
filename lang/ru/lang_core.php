...
'error_article_no'	=> 'Такая статья не найдена',//'文章不存在',
'liked_already'		=> 'Вы уже оценивали эту статью',//'已经赞过了',
'action_too_often'	=> 'Операция слишком частая',//'操作太频繁',
'invalid_request'	=> 'Недопустимый запрос',//'非正常请求',
'error_name_invalid'	=> 'Логин слишком длинный',//'昵称太长了',
...
'token_error'			=> 'Проверка токена не удалась. Попробуйте очистить файлы cookie браузера и обновить страницу или смените браузер и повторите попытку.',//'Token校验失败，请尝试清理浏览器cookie后刷新页面或者更换浏览器重试',
...
'dbtype_unsupported'		=> 'Данный тип БД не поддерживается: ',//'不支持的数据库类型: ',
'pdo_connect_error'		=> 'Ошибка подключения к базе данных. Проверьте введённую информацию. Сообщение об ошибке: ',//'连接数据库失败，请检查数据库信息。错误原因： ',
'sql_error'			=> 'Ошибка выполнения запроса:',//'SQL语句执行错误: ',
...
'useful_assistant'		=> 'Вы полезный ассистент',//'你是一个有用的助手',
'ai_model_not_configured'	=> 'Модель ИИ не сконфигурирована',//'AI 模型未配置',
'model_exception'		=> 'Ошибка обработки большой модели, попробуйте повторить позже. Сообщение об ошибке: ',//'大模型处理异常，请稍后再试，错误信息：',

//---------------------------------------
//include/service/media.php
'att_size_php_limit'		=> 'Размер файла превышает PHP лимит: ',//'文件大小超过PHP',
'att_type_disabled'		=> 'Данный тип файлов не разрешён к загрузке',//'不能上传该类型文件',
'att_size_system_limit'		=> 'Размер файла превышает системный лимит: ',//'文件太大了，系统限制上传：',
'att_upload_failed'		=> 'Загрузка не удалась, информация о файле не получена, вы можете сменить браузер и повторить попытку.',//'上传失败，未收到文件信息，可更换浏览器重试',
...
'tpl_setting_descr'	=> 'Это интерфейс настройки шаблона по умолчанию. Выберите пункт левого меню, чтобы войти в элементы настройки.',//'你好，这是默认模板的设置界面，请点击左侧菜单进入设置项。',
