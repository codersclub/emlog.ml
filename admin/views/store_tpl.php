...
<div class="d-flex flex-column flex-sm-row justify-content-between mb-4 ml-1">
    <div class="mb-3 mb-sm-0">
        <a href="./store.php?action=tpl" class="badge badge-primary m-1 p-2"><?= lang('all') ?></a>
        ...
        <a href="./store.php?action=tpl&tag=download_top" class="badge badge-light text-primary p-2 small"><?= lang('top_down') ?></a>
        <a href="./store.php?action=tpl&tag=paid_top" class="badge badge-light text-primary p-2 small"><?= lang('top_paid') ?></a>
    </div>
                                ...
                                <div>
                                    <?php if ($v['price'] > 0): ?>
                                        <?php if ($v['purchased'] === true): ?>
                                            ...
                                            <a href="#" class="btn btn-success installBtn" data-url="<?= urlencode($v['download_url']) ?>" data-cdn-url="<?= urlencode($v['cdn_download_url']) ?>" data-type="tpl"><?= lang('install_now') ?></a>
                                        <?php elseif ($v['svip'] && Register::getRegType() === 2): ?>
                                            <a href="#" class="btn btn-warning installBtn" data-url="<?= urlencode($v['download_url']) ?>" data-cdn-url="<?= urlencode($v['cdn_download_url']) ?>" data-type="tpl"><?= lang('install_now') ?></a>
                                        <?php else: ?>
                                            <a href="https://www.emlog.net/order/submit/tpl/<?= $v['id'] ?>" class="btn btn-danger" target="_blank"><?= lang('go_buy') ?></a>
                                        <?php endif ?>
                                    <?php else: ?>
                                        <a href="#" class="btn btn-success installBtn" data-url="<?= urlencode($v['download_url']) ?>" data-cdn-url="<?= urlencode($v['cdn_download_url']) ?>" data-type="tpl"><?= lang('install_free') ?></a>
                                    <?php endif ?>
                                </div>
                            </div>
                        </div>
